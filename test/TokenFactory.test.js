...
describe("TokenFactory", function () {
  ...
  describe("ساخت توکن رایگان (فقط owner)", function () {
    it("باید owner بتواند بدون پرداخت هیچ کمیسیونی توکن بسازد", async function () {
      const supply = ethers.parseUnits("1000", 18);
      const tx = await factory.connect(owner).createTokenFree("OwnerToken", "OWN", supply, owner.address);
      const receipt = await tx.wait();
      ...
      const event = receipt.logs
        ...
        .find((e) => e && e.name === "TokenCreated");
      expect(event).to.not.be.undefined;
      expect(event.args.paymentMethod).to.equal("FREE_OWNER");

      const token = await ethers.getContractAt("ERC20Template", event.args.tokenAddress);
      expect(await token.balanceOf(owner.address)).to.equal(supply);
    });

    it("باید owner بتواند توکن رایگان را به آدرس دلخواه دیگری هم مینت کند", async function () {
      const supply = ethers.parseUnits("500", 18);
      await factory.connect(owner).createTokenFree("GiftToken", "GIFT", supply, user.address);
      // آخرین رویداد را از طریق queryFilter بررسی می‌کنیم
      const events = await factory.queryFilter(factory.filters.TokenCreated());
      const lastEvent = events[events.length - 1];
      const token = await ethers.getContractAt("ERC20Template", lastEvent.args.tokenAddress);
      expect(await token.balanceOf(user.address)).to.equal(supply);
    });

    it("باید کاربر عادی (غیر owner) نتواند از این تابع استفاده کند", async function () {
      const supply = ethers.parseUnits("1000", 18);
      await expect(
        factory.connect(user).createTokenFree("HackToken", "HACK", supply, user.address)
      ).to.be.revertedWithCustomError(factory, "OwnableUnauthorizedAccount");
    });

    it("باید وقتی قرارداد pause شده، حتی owner هم نتواند رایگان بسازد", async function () {
      await factory.connect(owner).pause();
      const supply = ethers.parseUnits("1000", 18);
      await expect(
        factory.connect(owner).createTokenFree("PausedToken", "PSD", supply, owner.address)
      ).to.be.revertedWithCustomError(factory, "EnforcedPause");
    });
  });
